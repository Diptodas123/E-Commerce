...
const AddToCart = ({ product }) => {
  ...
  const setDecrease = () => {
    amount > 1 ? setAmount(amount => amount - 1) : setAmount(1);
  }

  const setIncrease = () => {
    amount < stock ? setAmount(amount => amount + 1) : setAmount(stock);
  }
        ...
        <p>Colors:
          {
            colors.map((currentColor, index) => {
              return (
                <button key={index} style={{ backgroundColor: currentColor }}
